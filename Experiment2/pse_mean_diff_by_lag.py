...
This script analyzes how the influence of extreme previous trials (10% vs 90% dot)
...
def get_rows_at_lag(df, values, lag):
    """
    Get rows at a specific lag after rows where 'dot_ratio' matches given values.
    ...
    """
    indices = df[df['dot_ratio'].isin(values)].index + lag
...
def analyze_pse_at_lag(file_path, lag):
    ...
    for extreme_ratio in EXTREME_RATIOS:
        ...
        dot_counts, total_counts = count_dot_choices(df_lagged, dot_res)
...
def plot_temporal_decay(delta_pse_by_lag):
    ...
    Args:
        delta_pse_by_lag: Dictionary mapping lag to delta PSE arrays
    """
    colors = ['green', 'yellowgreen', 'deepskyblue']
    ...
    # Add significance marker for 1 and 2-Back
    ax.text(0.3, 7, "**", ha='center', va='bottom', color='k', fontsize=20)
    ax.text(0.7, 7, "**", ha='center', va='bottom', color='k', fontsize=20)
...
def main():
    ...
    print(f"Comparing trials after {[int(r*100) for r in EXTREME_RATIOS]}% dot")

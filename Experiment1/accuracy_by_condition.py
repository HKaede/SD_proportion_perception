...
This script analyzes how the previous trial's stimulus (white ratio) affects
...
def calculate_accuracy(df, white_res, black_res):
    """
    ...
    for i in range(df.shape[0]):
        ...
        if ratio < 0.5 and response == black_res:
            correct += 1
        elif ratio > 0.5 and response == white_res:
            correct += 1
...
def analyze_sequential_effects(file_path):
    ...
    for prev_ratio in PREVIOUS_RATIOS:
        ...
        accuracy = calculate_accuracy(df_following, white_res, black_res)

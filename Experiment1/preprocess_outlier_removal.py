...
2. Accuracy in trials with a 10% or 90% proportion of white dots.
...
def get_next_rows(df, values):
    ...
    Args:
        ...
        values: List of values to match in 'white_ratio' column
...
def calculate_accuracy_skewed_trials(df, white_res, black_res):
    """
    Calculate accuracy for skewed rate trials (0.1 and 0.9).

    Args:
        df: DataFrame with trial data
        white_res: Response key for white dot choice
        black_res: Response key for black dot choice
    ...
    for i in range(df.shape[0]):
        white_ratio = df.iat[i, 0]
        response = df.iat[i, 1]
        
        # Check correctness based on trial type and participant assignment
        if white_ratio == 0.1:
            if (white_res == 'left' and response == 'right') or \
               (white_res == 'right' and response == 'left'):
                correct += 1
            amount += 1
        elif white_ratio == 0.9:
            if (white_res == 'left' and response == 'left') or \
               (white_res == 'right' and response == 'right'):
                correct += 1
            amount += 1
...
def main():
    ...
    for file_path in participants_valid_response:
        df, white_res, black_res = load_and_prepare_data(file_path)
        accuracy = calculate_accuracy_skewed_trials(df, white_res, black_res)
